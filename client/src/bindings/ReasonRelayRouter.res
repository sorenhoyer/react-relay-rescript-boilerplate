// Code c/p from https://github.com/zth/reason-relay-playground/blob/main/client/src/bindings/ReasonRelayRouter.re
module Route = {
  type t = {
...
module Url = {
  ...
  let toRouterUrl: t => ReasonReactRouter.url = t => {
    open RouterCompat
    {
...
let make = routes => {
  ...
  let currentRoute: ref<routeEntry> = ref({
    url: initialUrl,
  ...
  let _ = ReasonReactRouter.watchUrl(url =>
    if url != currentRoute.contents.url {
      ...
      subscribers->Js.Dict.values->Belt.Array.forEach(x =>
        switch x {
  ...
  let make = (
    ...
    let changeRoute = React.useCallback2(e => {
      ...
      | (false, 0, None | Some(#self), false) =>
        ...
        let navigate = () =>
          switch mode {
          | None | Some(#push) => ReasonReactRouter.push(to_)
    ...
    let preload = React.useCallback2(() => router.preload(to_), (to_, url.path))
